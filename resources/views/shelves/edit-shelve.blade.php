...
<div class="container w-25 " style="margin-top: 6rem">
    <div class="card h-150 d-fex ">
        <div class="bg-image hover-overlay ripple" data-mdb-ripple-color="light" style="border: 2px solid black; border-style: dashed" >
            <img src="{{ asset('image/shelve-edit.jpg') }}" class="img-fluid"/>
        </div>
        <div class="card-body p-0 py-4">
            <form action="{{ route('shelves.update', $shelve->id) }}" method="POST">
                ...
                <div class="mb-2">
                    <select id="blocks" name="block_id" class="form-select">
                        ...
                        @foreach($blocks as $block)
                            <option value="{{ $block->id }}" @if($block->id == $shelve->block_id) selected @endif>{{ $block->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-outline mb-2">
                    <input value="{{ $shelve->name }}" name="name" type="text" id="shelve" class="form-control" />
                ...
                @endif
                <!-- Submit button -->
                <button type="submit" class="btn btn-outline-warning btn-block">Update Shelve</button>
            </form>
        </div>
    </div>
</div>
